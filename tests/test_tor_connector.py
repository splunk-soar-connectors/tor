...
class RecentExitTests(unittest.TestCase):
    def setUp(self):
        self.connector = TordnselConnector.__new__(TordnselConnector)  # pylint: disable=no-value-for-parameter
    ...
    def test_recent_exits_are_queried_per_lookup(self):
        ...
        request.assert_called_once_with("https://check.torproject.org/cgi-bin/TorBulkExitList.py?ip=198.51.100.2", timeout=30)
